...
export const CACHE_TAGS = {
  ARTICLE: {
    ...
    getSearchArticleList: (params: GetSearchArticleListParams) =>
      CACHE_TAGS_PREFIX.ARTICLE + `getSearchArticleList-${JSON.stringify(params)}`,
    getBookmarkList: (userId: number, params: GetBookmarkListParams) =>
      CACHE_TAGS_PREFIX.ARTICLE + `getBookmarkList-${userId}-${JSON.stringify(params)}`,
    ...
    getArticleList: (params: GetArticleListParams) =>
      CACHE_TAGS_PREFIX.ARTICLE + `getArticleList-${JSON.stringify(params)}`,
    getArticleListByUserId: (userId: number, params: GetArticleListByUserIdParams) =>
      CACHE_TAGS_PREFIX.ARTICLE + `getArticleListByUserId-${userId}-${JSON.stringify(params)}`
  },
  ARTICLE_SCHEDULE: {
    ...
    getSchedulePlaceList: (articleId: number) =>
      CACHE_TAGS_PREFIX.ARTICLE_SCHEDULE + `getSchedulePlaceList-${articleId}`
  },
  USER_PROFILE: {
